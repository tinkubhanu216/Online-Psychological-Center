...
<body>
	...
	<center>
		...
		<form class="ui form" action="acceptappointment.php" method="get">
			<div class="fields">
		...
		<?php
			$sql="select *from appointment where status='pending' order by time";
			...
			if(mysqli_num_rows($result)>0){
				while($row=mysqli_fetch_assoc($result)){
					...
					$requestingtime=$row["requestingtime"];
					$token=$row["token"];
				...
				<div class='four wide field'>
					<input type='text' value='$requestingdate' style='padding:10px' >
				</div>
				<div class='four wide field'>
					<input type='text' value='$requestingtime' style='padding:10px' >
				</div>
				<div class='two wide field'>
					<button type='submit' value='$token' name='token' class='ui button'>Accept</button>					
				</div>
			</div>";
				}
			}
